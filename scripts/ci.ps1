...
function Invoke-CiStep {
  ...
  try {
    ...
    $seconds = ((Get-Date) - $stepStart).TotalSeconds
    Add-Result -Name $Name -Status 'pass' -ExitCode 0 -Seconds $seconds
    Write-Host "PASS $Name"
  } catch {
    $seconds = ((Get-Date) - $stepStart).TotalSeconds
    Add-Result -Name $Name -Status 'fail' -ExitCode 1 -Seconds $seconds
